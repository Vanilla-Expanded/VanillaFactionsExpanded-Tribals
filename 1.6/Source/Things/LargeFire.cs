    ...
    public class LargeFire : Building_WorkTable
    {
        ...
        protected void SpawnSpark()
        {
            IntVec3 position = GenRadial.RadialCellsAround(Position, 2.9f, true)
                .Where(x => x != Position).RandomElement();
            if (!position.InBounds(Map))
            {
                return;
            }
            CellRect startRect = CellRect.SingleCell(Position);
            CellRect endRect = CellRect.SingleCell(position);
            if (GenSight.LineOfSight(Position, position, Map, startRect, endRect))
            {
                ((Spark)GenSpawn.Spawn(VFET_DefOf.VFET_LargeFireSpark, Position, Map)).Launch(this, position, position, ProjectileHitFlags.All);
            }
        }
